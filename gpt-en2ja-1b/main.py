...
def main(ModelArguments, DataTrainingArguments, TrainingArguments):
    ...
    if training_args.push_to_hub:
        HfApi().create_repo(
            ...
            organization=os.getenv("HF_HUB_ORG"),
            private=True,
            exist_ok=True,
        )
    ...
    def tokenize_function(examples):
        ...
        if "Token indices sequence length is longer than the" in cl.out:
            tok_logger.warning(
                "^^^^^^^^^^^^^^^^ Please ignore the warning above - this long input will be chunked into smaller bits before being passed to the model."  # noqa
            )
        return output
